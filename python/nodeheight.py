...
def minHeightNodes(nodes, edges):
    ...
    for start in range(0, nodes):
        ...
        coverage = set([start])
        testHeight = 1
        while(len(coverage) < nodes):
            testHeight += 1
            levelHeight = set()
            # Before we have full coverage, check all edges
            for edge in edges:
                if edge[0] in coverage and edge[1] in coverage:
                    continue
                elif edge[0] in coverage or edge[1] in coverage:
                    # Add each edge as "covered" on this level
                    levelHeight = levelHeight.union(edge)
                else:
                    # Edge cannot be reached, so do nothing
                    pass
                    
            # Combine levels to covered
            coverage = coverage.union(levelHeight)
        
        print(start, testHeight)
        if testHeight < minHeight:
            # Replace possible start and height
            minHeight = testHeight
            minHeightNodes = [start]
        elif testHeight == minHeight:
            # Append possible start to list
            minHeightNodes.append(start)

    return minHeightNodes

print(minHeightNodes(nodes, edges))
